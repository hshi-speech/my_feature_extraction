...
def linear_spectrogram(waveform: Tensor,
    ...
    spectrum = torch.max(fft.abs(), epsilon)  # size (m, padded_window_size // 2 + 1)
    # spectrum[:, 0] = signal_log_energy

    spectrum = _subtract_column_mean(spectrum, subtract_mean)
    return spectrum, padded_window_size
...
def spectrum_trans_fbank(spectrum: Tensor,
          padded_window_size: Tensor,
          blackman_coeff: float = 0.42,
          channel: int = -1,
          dither: float = 0.0,
          energy_floor: float = 1.0,
