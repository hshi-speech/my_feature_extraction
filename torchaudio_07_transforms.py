...
__all__ = [
    ...
    'AmplitudeToDB',
    'MelScale',
    'Spectrogram_trans_MelSpectrogram',
    'MelSpectrogram_trans_MFCC',
    'InverseMelScale',
    'MelSpectrogram',
    'MFCC',
    'MuLawEncoding',
...
class inverseSTFT(torch.nn.Module):
    ...
    def forward(self, complex_stft: Tensor, length: int) -> Tensor:
        r"""
        Args:
            waveform (Tensor): Tensor of audio of dimension (..., time).
        ...
        Returns:
            ...
            Fourier bins, and time is the number of window hops (n_frame).
        """
        inverse = torch.istft(complex_stft,
                              n_fft=self.n_fft,
...
class Spectrogram_trans_MelSpectrogram(torch.nn.Module):
    r"""Create MelSpectrogram for a raw audio signal. This is a composition of Spectrogram
    and MelScale.
    ...
    def __init__(self, sample_rate=16000, n_fft=400, win_length=None, hop_length=None, f_min=0., f_max=None,
        ...
        super(Spectrogram_trans_MelSpectrogram, self).__init__()
        self.sample_rate = sample_rate
        self.n_fft = n_fft
        self.win_length = win_length if win_length is not None else n_fft
        self.hop_length = hop_length if hop_length is not None else self.win_length // 2
...
class MelSpectrogram_trans_MFCC(torch.nn.Module):
    r"""Create the Mel-frequency cepstrum coefficients from an audio signal
    ...
    def __init__(self, sample_rate=16000, n_mfcc=40, n_mels=128, dct_type=2, norm='ortho', log_mels=False,
                 melkwargs=None):
        super(MelSpectrogram_trans_MFCC, self).__init__()
        supported_dct_types = [2]
        if dct_type not in supported_dct_types:
            raise ValueError('DCT type not supported'.format(dct_type))
        self.sample_rate = sample_rate
        ...
        dct_mat = F.create_dct(self.n_mfcc, self.n_mels, self.norm)
        self.register_buffer('dct_mat', dct_mat)
        self.log_mels = log_mels
    ...
    def forward(self, mel_specgram):
        ...
        """
        shape = mel_specgram.size()
        mel_specgram = mel_specgram.reshape(-1, shape[-2], shape[-1])
        ...
        # (channel, n_mels, time).tranpose(...) dot (n_mels, n_mfcc)
        # -> (channel, time, n_mfcc).tranpose(...)
        mfcc = torch.matmul(mel_specgram.transpose(1, 2), self.dct_mat.to(device)).transpose(1, 2)

        # unpack batch
        mfcc = mfcc.reshape(shape[:-2] + mfcc.shape[-2:])
